...
| 编号 | 需求 | 优先级 |
|------|------|--------|
| F-DL-01 | 用户可创建每日定时器，设置触发时间（HH:MM，**用户本地时间**）和时区 | 高 |
| F-DL-02 | 定时器每天在指定本地时间触发一次通知，后端自动完成时区转换 | 高 |
| F-DL-03 | 每日定时器触发后状态保持 `active`，等待下次触发 | 高 |
| F-DL-04 | 服务重启后，每日定时器自动恢复调度 | 中 |
| F-DL-05 | 支持 IANA 时区名称（如 `Asia/Shanghai`、`America/New_York`），正确处理夏令时 | 高 |
| F-DL-06 | Web 控制台根据浏览器自动检测时区，并支持用户手动选择 | 中 |
| F-DL-07 | 控制台显示的"下次触发时间"以用户所选时区展示 | 中 |
...
| NF-05 | 时区 | 内部存储使用 UTC；每日定时器接收用户本地时间 + IANA 时区，后端用 `zoneinfo` 转换，APScheduler cron 以用户时区运行以正确处理夏令时 |
